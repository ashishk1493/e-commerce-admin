...
import EcomProductAdd from 'src/components/Bulk/EcomProductAdd';
import { get_category_list_autocomplete_slice, get_category_list_slice } from 'src/redux/slices/ecom_category';
import { add_product_service, edit_product_service, getbyid_product_service } from 'services/ecom_product.service';
...
export default function addProduct(props) {
    ...
    const onLoad = async () => {
        ...
        // console.log(objUser, "resProduct");
    };
    ...
    return (
        <FormCover
            ...
            breadcomeTitle={'Edit Product'}
            breadcomeLinks={[{ name: 'Home', href: PATH_DASHBOARD.root }, { name: 'Edit Product' }]}
        > {id && objUserDetails.category_id ?
            <FormikCommanNew
                ...
                onSubmitProps={async (values) => {
                    ...
                    setLoading(true);
                    let res = await anqEditform(values, id, edit_product_service, redirectOnSuc);
                    setLoading(false);
                }}
                Ch={EcomProductAdd}
            />
